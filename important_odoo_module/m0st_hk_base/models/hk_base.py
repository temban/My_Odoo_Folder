...
##        mydict.items()
    ...
    def format_date ( self, mydate, is_datetime=False, separateur='-', sep_return='/' ):
        ...
        res1 = {  'jour': tdate [ 2 ], 'mois': tdate [ 1 ], 'annee': tdate [ 0 ],
                  ...
                  'complet': tdate [ 2 ] + sep_return + tdate [ 1 ] + sep_return + tdate [ 0 ]+" "+ttime[0]+":"+ttime[1]+":"+ttime[2],
                }
...
class LuggageType(models.Model):
    ...
    def _compute_code(self):
        for r in self:
            ...
            if r.type == 'envelope': code = u"ENV - %s" % r.id
            elif r.type == 'briefcase': code = u"BRB - %s" % r.id
            elif r.type == 'suitcase': code = u"STC - %s" % r.id
