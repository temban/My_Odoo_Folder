...
async def main():
    ...
    ssl_context.load_cert_chain(
        pathlib.Path('/etc/letsencrypt/live/preprod.hubkilo.com/cert.pem'),  # Path to your SSL certificate
        pathlib.Path('/etc/letsencrypt/live/preprod.hubkilo.com/privkey.pem')  # Path to your private key
    )
    server = websockets.serve(handle_connection, "0.0.0.0", 9090, ssl=ssl_context)
