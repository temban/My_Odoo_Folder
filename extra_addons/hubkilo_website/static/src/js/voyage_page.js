...
const pattern = /^\/booking\/page\/road\/(\d+)$/;
const patternFR = /^\/fr\/booking\/page\/road\/(\d+)$/;

// Check if the current path matches the pattern
const match = window.location.pathname.match(pattern);
const matchFR = window.location.pathname.match(patternFR);
...
//    console.log(options);
  ...
  var selectElement = document.getElementById("destinataire");
...
var email = options.email === false ? "No Email" : options.email
  ...
  return `<div class="col-md-6"><div id="my_reserve_button${options.id}" onclick="myBooking2(${options.id},'${options.name}',5)" style="background:white;display:flex;align-items:center;gap:20px;margin-top:5px;box-shadow:1px 10px 30px rgba(126, 124, 124, 0.24);padding:10px;margin:5px">
               <div>
...
//Travel infor user image
var travelUser = []

 axios.get(`/api/v1/read/m1st_hk_roadshipping.travelbooking?ids=${elementId}`, {
        headers: {
          'Accept': 'application/json',
          'Authorization': authorization,
        },
...
//User
    travelUser.push(response.data[0])
...
//    console.log(myItem)
    ...
    localStorage.setItem("travelbooking_id",myItem.id)
      ...
      const travel_info_details = document.getElementById("travel_info_details1");
 ...
 travel_info_details.innerHTML = `
                  <div class=""
                     ...
                     <p style="white-space: nowrap;overflow: hidden;;font-size:17px;text-overflow: ellipsis;width:120px"
                        ...
                        <p style="white-space: nowrap;overflow: hidden;font-size:12px;font-weight:300;text-overflow: ellipsis;width:135px"
                        class="heading text-center">${formattedDate} ${dtime}</p>
                  </div>
                  ...
                  <div class=" "
                     ...
                     <p style="white-space: nowrap;overflow: hidden;font-size:17px;text-overflow: ellipsis;width:120px"
                      ...
                      <p style="white-space: nowrap;overflow: hidden;font-size:12px;font-weight:300;text-overflow: ellipsis;width:135px"
                        class="heading text-center">${formattedDate2} ${atime}</p>
               </div>`;

travel_info_state.innerHTML = `<h5 class="d-flex justify-content-md-start justify-content-center" style="font-weight:800; font-size:22px;color: black; margin-top: 20px; margin-bottom: 1px;">
                  ${myItem.code}
               </h5>
               <div class="d-flex justify-content-md-start justify-content-center">
               <img  src="/hubkilo_website/static/src/media/road-trip.gif" style="width:130px;height:130px" alt="road trip"/>
               </div>
              `
...
function createUserAndManual(section){
...
return `  <div class="col-lg-6">
        ...
        <div id="my_reserve_button${item.id}" onclick="myBooking2(${item.id},'${item.name}',5)"  style="display:flex;align-items:center;gap:20px;margin-top:10px;box-shadow:1px 10px 30px rgba(126, 124, 124, 0.24);padding:10px;margin:20px">
               <div>
...
if (currentPath1 === '/add/new/shipping' || match || matchFR) {
...
function myBooking1(event,section){
