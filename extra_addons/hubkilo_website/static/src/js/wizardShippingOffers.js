...
function verifyDestination(section){
        if(document.getElementById("ville_de_depart").value.trim() === ""|| document.getElementById("date_de_depart_offer").value.trim() === "" || document.getElementById("ville_de_arriver").value.trim() === "" || document.getElementById("date_de_arrivee_offer").value.trim() === ""){
                   var alert_error_message = document.getElementById("alert_error_message")
   alert_error_message.innerHTML = `<p style="text-transform:capitalize;font-weight:300">All the fields are required</p>`
