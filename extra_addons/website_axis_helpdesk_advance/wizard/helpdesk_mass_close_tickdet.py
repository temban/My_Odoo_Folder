    ...
    def _compute_user_and_stage_ids(self):
        for ticket in self.filtered(lambda ticket: ticket.team_id):
            if not ticket.user_id:
                ticket.user_id = ticket.helpdesk_team_id._determine_user_to_assign()[ticket.helpdesk_team_id.id]
            if not ticket.helpdesk_stage_id or ticket.helpdesk_stage_id not in ticket.helpdesk_team_id.stage_ids:
                ticket.helpdesk_stage_id = ticket.helpdesk_team_id._ticket_stage_define()[ticket.team_id.id]
    ...
    def update_ticket(self):
        ...
        if self.update_for == "close_tickets":
            for ticket in selected_records:
                ticket.update({'helpdesk_stage_id': self.stage_id.id})
        elif self.update_for == "update_team_user":
            for ticket in selected_records:
                ticket.update({'team_id': self.team_id.id,
