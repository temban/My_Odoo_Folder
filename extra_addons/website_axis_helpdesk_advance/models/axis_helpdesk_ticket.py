...
class axisHelpdeskTicket(models.Model):
    ...
    helpdesk__sla_deadline = fields.Datetime(string="SLA deadline")
    ...
    @api.depends('helpdesk__sla_deadline', 'helpdesk__sla_late')
    def _compute_ticket_sla_policy_fail(self):
        now = fields.Datetime.now()
        for ticket in self:
            if ticket.helpdesk__sla_deadline:
                ticket.ticket_sla_policy_fail = (ticket.helpdesk__sla_deadline < now) or ticket.helpdesk__sla_late
            else:
    ...
    def _compute_ticket_sla_policy_success(self):
        now = fields.Datetime.now()
        for ticket in self:
            ticket.ticket_sla_policy_success = (ticket.helpdesk__sla_deadline and ticket.helpdesk__sla_deadline > now)
        ...
        for ticket in self:
            ...
            for policy in ticket.sla_ids:
                if policy.days or policy.hours or policy.time_minutes:
                    ...
                    ticket.write({'helpdesk__sla_deadline': deadline})
    ...
    def action_view_invoice(self):
        ...
        action = {
            'type': 'ir.actions.act_window',
            'domain': [('id', 'in', self.account_invoice_ids.ids),('type', '=', 'out_invoice')],
            'view_mode': 'list,form',
            'name': ('Invoices'),
            'res_model': 'account.move',
        }
    ...
    def _onchange_team(self):
        ...
        if self.based_on_ticket_type:
            if self.helpdesk_ticket_type_id.type_based_on == "helpdesk_team":
                ...
                self.helpdesk_team_id = self.helpdesk_ticket_type_id.team_id
    ...
    def send_whatsapp_msg(self):
        return {'type': 'ir.actions.act_window',
                ...
                'view_mode': 'form',
                'view_type': 'form',
                'context': {'default_res_user_id': self.partner_id.id,
