...
class axisHelpdeskTeam(models.Model):
    ...
    def _compute_fail_sla_tickets(self):
        ticket_id = self.env['axis.helpdesk.ticket'].read_group([
            ...
            ('helpdesk__sla_deadline', '!=', False),
            ('helpdesk__sla_deadline', '<=', fields.Datetime.to_string((date.today() + relativedelta.relativedelta(days=1)))),
    ...
    def filter_stage_data_dashboard(self):
        ...
        sla_complete = self.env['axis.helpdesk.ticket'].search_read(domain=[('helpdesk_stage_id', '=', 'Done'),('helpdesk__sla_deadline','<=',datetime_object)])
        sla_pending = self.env['axis.helpdesk.ticket'].search_read(domain=[('helpdesk_stage_id', '!=', 'Done'),('helpdesk__sla_deadline','>=',datetime_object)])
        sla_missed = self.env['axis.helpdesk.ticket'].search_read(domain=[('helpdesk_stage_id', '!=', 'Done'),('helpdesk__sla_deadline','<=',datetime_object)])
        ...
        user_uses_sla = self.user_has_groups('helpdesk.group_use_sla') and \
                        bool(self.env['axis.helpdesk.ticket.team'].search(
                            [('use_sla', '=', True), '|', ('res_user_ids', 'in', self._uid), ('res_user_ids', '=', False)]))
        user_id = self.env['res.users'].search_read([])

        team_id = self.env['axis.helpdesk.ticket.team'].search_read([])
        ...
        team_data = self.env['axis.helpdesk.ticket'].search([('res_user_id', '=', 8), ('helpdesk_team_id', '=', 2)])
        if self.env['ir.config_parameter'].sudo().get_param('stage_ids'):
            ...
            stage_ids = self.env['axis.helpdesk.stage'].browse(stage_ids)
        else:
            stage_ids = []
        ...
        if user_uses_sla:
            group_fields.insert(1, 'helpdesk__sla_deadline:year')
            group_fields.insert(2, 'helpdesk__sla_deadline:hour')
            group_fields.insert(3, 'helpdesk__sla_late')
            list_fields.insert(1, 'helpdesk__sla_deadline')
        ...
        tickets = HelpdeskTicket.search_read(expression.AND([domain, [('helpdesk_stage_id.closed', '=', False)]]),
                                             ['helpdesk__sla_deadline', 'closed_hours', 'helpdesk__sla_late', 'priority'])
        ...
        stage_ids = self.env['axis.helpdesk.stage'].search_read([('id', 'in', user_company_id.helpdesk_stage_ids.ids)])
        cr = self._cr
        ...
        for ticket_data in ticket_id_all_data:
            stage_id = self.env['axis.helpdesk.stage'].search([('id', '=', ticket_data['helpdesk_stage_id'])])
            partner_id = self.env['res.partner'].search([('id', '=', ticket_data['partner_id'])])
            user_id = self.env['res.users'].search([('id', '=', ticket_data['res_user_id'])])
            ticket_dic = {
        ...
        for stage_id in stage_ids:
            ticket_id = self.env['axis.helpdesk.ticket'].search([('helpdesk_stage_id', '=', stage_id['id'])])
            data_dic = {
        ...
        def failed_sla_ticket(data):
            deadline = data.get('helpdesk__sla_deadline')
            helpdesk__sla_deadline = fields.Datetime.now() > deadline if deadline else False
            return helpdesk__sla_deadline or data.get('helpdesk__sla_late')
