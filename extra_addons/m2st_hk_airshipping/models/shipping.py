...
class Luggage(models.Model):
    _name = 'm2st_hk_airshipping.luggage'
    _inherit = 'm0sthk.base'
    _description = "Shipping Luggages"
    ...
    ##-------------- FIELDS
    name = fields.Text(string="Luggage desc", required=True, default='/')

    luggage_image1 = fields.Image("Internal Image 1", max_width=512, max_height=512, )

    luggage_image2 = fields.Image("Internal Image 2", max_width=512, max_height=512, )

    luggage_image3 = fields.Image("Internal Image 3", max_width=512, max_height=512, )

    luggage_image4 = fields.Image("Internal Image 4", max_width=512, max_height=512, )

    luggage_image5 = fields.Image("Internal image 5", max_width=512, max_height=512, )

    luggage_image6 = fields.Image("Internal Image 6", max_width=512, max_height=512, )

    luggage_image7 = fields.Image("External Image 7", max_width=512, max_height=512, )

    luggage_image8 = fields.Image("External image 8", max_width=512, max_height=512, )

    luggage_image9 = fields.Image("External Image 9", max_width=512, max_height=512, )

    luggage_image10 = fields.Image("External Image 10", max_width=512, max_height=512, )

    luggage_image11 = fields.Image("External image 11", max_width=512, max_height=512, )

    luggage_image12 = fields.Image("External Image 12", max_width=512, max_height=512, )

    shipping_id = fields.Many2one('m2st_hk_airshipping.shipping', string="Shipping", )

    travelbooking_id = fields.Many2one(
        'm2st_hk_airshipping.travelbooking',
        string="Travel booking",
        related='shipping_id.travelbooking_id',  # Link to travelbooking_id of the associated shipping
        store=True,  # Enable storing the value in the database
    )

    luggage_model_id = fields.Many2one(
        'm2st_hk_airshipping.flight.luggage',
        string="Luggage model",
        required=True,
        domain="[('flight_announcement_id', '=', travelbooking_id)]",  # Add this domain
    )

    # Add the fields with the compute method
    average_width = fields.Float(string="Width (meter)", compute='_compute_envelop_dimensions', store=True,
                                 help="This is the average width for a type of luggage expressed in meters")
    average_length = fields.Float(string="Length (meter)", compute='_compute_envelop_dimensions', store=True,
                                  help="This is the average length for a type of luggage expressed in meters")
    average_height = fields.Float(string="Height (meter)",
                                  help="This is the average height for a type of luggage expressed in meters")
    average_weight = fields.Float(string="Weight (kilo gram)",
                                  help="This is the average weight expressed in kg")
    average_size = fields.Float(string="Size (meter square or cube)", compute='_compute_dimensions', readonly=True,
                                store=True, help="This is the average size for a type of luggage expressed in meters")

    color = fields.Integer(string='Color Index', default=_get_default_color)

    quantity_to_transfer = fields.Float(string="Quantity", default=1,
                                        help="This is the number of corresponding to what you want to transfer")

    envelop_formate = fields.Selection(ENVELOPE_FORMAT_SELECTION,
                                       string='Envelope Format',
                                       widget='selection')

    luggage_type_id = fields.Many2one('m2st_hk_airshipping.luggage.type', compute='_compute_luggage_fields', store=True)
    price = fields.Monetary(string='Unit Price', compute='_compute_luggage_fields', store=True)
    quantity = fields.Float(string='Total Quantity', compute='_compute_luggage_fields', store=True)
    is_kilo_luggage = fields.Boolean(compute='_compute_luggage_fields', store=True)
    luggage_name = fields.Char(compute='_compute_luggage_fields', store=True)

    @api.depends('envelop_formate', 'average_width', 'average_length', 'average_height')
    def _compute_dimensions(self):
        for luggage in self:
            if luggage.envelop_formate and luggage.luggage_name == 'ENVELOP':
                dimensions = {
                    'A0': (0.84, 0.12),
                    'A1': (0.59, 0.84),
                    'A2': (0.42, 0.59),
                ...
                luggage.average_width = dimensions[luggage.envelop_formate][0]
                luggage.average_length = dimensions[luggage.envelop_formate][1]
                luggage.average_size = luggage.average_width * luggage.average_length
            else:
                luggage.average_size = luggage.average_width * luggage.average_length * luggage.average_height

    @api.onchange('quantity_to_transfer')
    def _check_quantity_to_transfer(self):
        for record in self:
            if record.luggage_name != '' and record.is_kilo_luggage and record.quantity_to_transfer > record.quantity:
                self.quantity_to_transfer = 1.0
                text = _(
                    u"Quantity to transfer cannot be more than traveler total quantity. if not adjusted, the transfer quantity will be reduce to 1.0")
    ...
    @api.depends('luggage_model_id')
    def _compute_luggage_fields(self):
        for luggage in self:
            if luggage.luggage_model_id:
                luggage.luggage_name = luggage.luggage_model_id.luggage_type_id.name
                luggage.luggage_type_id = luggage.luggage_model_id.luggage_type_id
                luggage.price = luggage.luggage_model_id.price
                luggage.quantity = luggage.luggage_model_id.quantity
                luggage.is_kilo_luggage = luggage.luggage_model_id.is_kilo_luggage
...
class RoadShipping(models.Model):
    ...
    def _get_default_name(self):
        ...
        if ship_format_code == 'ship_year_num':
            return u"AIRSHIP/%s/%s" % (time.strftime('%Y'), count)
        elif ship_format_code == 'ship_num_year':
            return u"AIRSHIP/%s/%s" % (count, time.strftime('%Y'))
        else:
            return u"%s/AIRSHIP/%s" % (count, time.strftime('%Y'))
    ...
    def _compute_booking_type(self):
        for r in self:
            if not r.travelbooking_id:
                r.booking_type = ''
            elif r.travelbooking_id.booking_type == 'air':
                r.booking_type = 'By Air'
    ...
    travelbooking_id = fields.Many2one('m2st_hk_airshipping.travelbooking',
                                       string="Travel booking", required=True, readonly=True,
    ...
    luggage_ids = fields.One2many('m2st_hk_airshipping.luggage', inverse_name='shipping_id', string="Lugagges",
                                  required=True, )

    total_weight = fields.Float(string="Average total weight",
    ...
    # state_id = fields.Many2one("res.city", string='State', ondelete='restrict', required=True,
    ...
    luggage_cost = fields.Monetary(string="Luggage Cost", digits=(16, 2),
                                   readonly=True, store=True, currency_field='currency_id', )

    amount_deducted = fields.Monetary(string="Administrative Fee", digits=(16, 2),
                                      readonly=True, store=True, currency_field='currency_id',
                                      help="this is the amount that will be deducted as a transaction fee", )

    total_cost = fields.Monetary(string="Transaction Cost", digits=(16, 2),
                                 readonly=True, store=True, currency_field='currency_id',
                                 help="this is the global amount of operation \
                                        \nTransaction Cost = shipping_price + administrative fee", )
    ...
    payment_link = fields.Char(related='move_id.air_payment_link', store=True,
    ...
    @api.depends('luggage_ids')
    def _compute_products(self):
        for shipping in self:
            products = self.env['product.product']
            for luggage in shipping.luggage_ids:
                product = self.env['product.product'].search([('default_code', '=', 'HBK-' + luggage.luggage_name)],
                                                             limit=1)
                if product:
                    products |= product
            shipping.product_ids = products
    ...
    def onchange_partner(self):
        ...
        for receiver in my_partner.mapped('air_receiver_partner_ids'):
    ...
    def pay_shipping(self):
        ...
        percentage = resconfigvalues['hk_base_config_air_admin_fee_percentage']
        administrative_fee_unit_price = 0
        air_shipping_price = 0
        ...
        administrative_fee_product = product_obj.search([('default_code', '=', 'HBK-ADMIN-FEE')], limit=1)

        ##----------------- INV FOR HUBKILO
        pbank_ids = self.partner_id.mapped('bank_ids')
        invoice_lines = []  # Move this line outside the outer loop

        for shipping in self:
            for luggage in shipping.luggage_ids:
                hbk_sh_product = product_obj.search([('default_code', '=', 'HBK-' + luggage.luggage_name)],
                                                    limit=1)
                print("product:", hbk_sh_product.name, luggage.quantity_to_transfer, luggage.price)
                ...
                invl_vals = {
                    ...
                    'name': hbk_sh_product.name, 'quantity': luggage.quantity_to_transfer,
                    'price_unit': luggage.price, 'discount': 0.0,
                ...
                invoice_lines.append((0, 0, invl_vals))

                administrative_fee_unit_price += (float(percentage) / 100) * float(luggage.price) * float(luggage.quantity_to_transfer)
                air_shipping_price += float(luggage.price) * float(luggage.quantity_to_transfer)
                administrative_fee += (float(percentage) / 100) * float(luggage.price) * float(luggage.quantity_to_transfer)
                total += air_shipping_price + administrative_fee

        admin_fee_invl_vals = {
            'name': administrative_fee_product.name,
            'product_uom_id': administrative_fee_product.uom_id.id, 'product_id': administrative_fee_product.id,
            'date_maturity': fields.Date.today(), 'currency_id': self.currency_id.id,
            'quantity': 1,
            'price_unit': administrative_fee_unit_price,
            'account_id': administrative_fee_product.property_account_income_id.id,
        }
        invoice_lines.append((0, 0, admin_fee_invl_vals))

        print("product:", invoice_lines)
        inv_vals = {
            ...
            'air_travelbooking_id': self.travelbooking_id.id,
            'air_shipping_id': self.id,
        }
        ...
        total = air_shipping_price + administrative_fee
        self.write({'state': 'accepted', 'move_id': hubkilo_inv.id, 'shipping_price': air_shipping_price,
                    'amount_deducted': administrative_fee, 'total_cost': total})
    ...
    def set_to_paid(self):
        ...
        for r in self:
            for luggage in r.luggage_ids:
                if luggage.is_kilo_luggage:
                    for booking in r.travelbooking_id.luggage_types:
                        if booking.is_kilo_luggage:
                            booking.quantity -= luggage.quantity_to_transfer
                            luggage.quantity -= luggage.quantity_to_transfer
        return self.write({'is_paid': True, 'state': 'paid'})  # 'move_id': hubkilo_inv.id,
        print("oooooooooooooooooooooo1")
...
class AccountMove(models.Model):
    ...
    air_travelbooking_id = fields.Many2one('m2st_hk_airshipping.travelbooking', string="Travel booking",
                                           readonly=True, )

    air_shipping_id = fields.Many2one('m2st_hk_airshipping.shipping', string="Shipping",
                                      readonly=True, )

    air_payment_link = fields.Char(string="Payement Link", readonly=True, )

    @api.model
    def mark_air_shipping_as_paid(self):
        ...
        for inv in invoices:
            if inv.air_shipping_id:
                if inv.air_shipping_id.state in ['accepted']:
                    inv.air_shipping_id.set_to_paid()
                    print("oooooooooooooooooooooo2")

    # @api.model
    # def mark_air_shipping_as_paid(self, move):
    #     for move in self:
    #         if move.payment_state == 'paid' and move.move_type == 'out_invoice' and move.air_shipping_id and move.air_shipping_id.state == 'accepted':
    #             move.air_shipping_id.set_to_paid()



    # def _update_shipping_state(self, ):
    #     if move.state == 'posted' and move.payment_state == 'paid':
    #         shipping = self.env['m1st_hk_roadshipping.shipping'].search([('move_id', '=', move.id)])
    #         if shipping:
    #             shipping.write({'state': 'paid', 'is_paid': True})

    def write(self, vals):
        res = super(AccountMove, self).write(vals)
        self.mark_air_shipping_as_paid()
        return res
    ...
    def _force_to_generate_payment_link(self):
        ...
        self.air_payment_link = plwiz_rec.link
