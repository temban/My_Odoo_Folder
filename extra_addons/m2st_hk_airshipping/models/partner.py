...
class resPartner(models.Model):
    ...
    ##--------------------- FIELDS
    air_shipping_ids = fields.One2many('m2st_hk_airshipping.shipping',
                                    inverse_name='partner_id', string="Shippings", readonly=True, )

    air_travelbooking_ids = fields.One2many('m2st_hk_airshipping.travelbooking',
                                        inverse_name='partner_id', string="Travels", readonly=True, )


    air_receiver_partner_ids = fields.Many2many(comodel_name='res.partner', compute='_air_get_receivers', string="My Receivers")


    directory_partner_ids = fields.Char(compute='_air_get_receivers', string="Partner directory IDS",
                                             store=True, )

    directory_partner_names = fields.Char(compute='_air_get_receivers', string="Partner directory Names",
                                        store=True, )

    directory_partner_list_ids = fields.Char(compute='_air_get_receivers', string="Partner directory List IDS",
                                        store=True, )
    ...
    def _air_get_receivers(self):
        ship_obj = self.env.get('m2st_hk_airshipping.shipping')
        for r in self:
            all_shippings = ship_obj.search([('partner_id', '=', r.id), ('state', 'not in', ['rejected'])])
            air_receiver_partner_ids        = all_shippings.mapped('receiver_partner_id') or None
            directory_partner_list_ids  = ''
            directory_partner_names     = ''
            seq                         = 0

            if air_receiver_partner_ids is not None:
                for dpid in air_receiver_partner_ids:
                    if seq == 0 :
            ...
            r.update( {'air_receiver_partner_ids': air_receiver_partner_ids,
