...
class ir_attachment (models.Model):
    ...
    def set_and_delete_invalid_attachments(self):
        ...
        # Filter attachments based on attach_custom_type
        attachments = self.sudo().search([('attach_custom_type', 'in', ['cni', 'passport'])])

        for attachment in attachments:
            if attachment.date_end and today > attachment.date_end:
                attachment.write({'validity': False, 'conformity': False})

        _logger.info("Deleting invalid attachments...")

        # Filter invalid attachments based on attach_custom_type
        invalid_attachments = attachments.filtered(
            lambda att: not att.validity and not att.conformity
        )

        _logger.info("Invalid attachments found: %s", invalid_attachments)
        print("Deleting expired attachments")
        ...
        for days_remaining in range(3, 0, -1):
            ...
            for attachment in attachments:
                template = self.env.ref(
                    'm0st_hk_base.hubkilo_send_attachment_expiry_notification')  # Replace 'your_module.email_template_id' with the actual email template ID
                if template:
